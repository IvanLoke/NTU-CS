...
int calculatetoll(BTNode* root, int total, int n1, int n2){
    ...
    int leftside =calculatetoll(root->left,total,n1,n2);
    int rightside = calculatetoll(root->right,total,n1,n2);

    if (leftside != NULL && rightside != NULL){
        total = leftside + rightside + root->nodeV;
        return total;
    }
    if (leftside == NULL & rightside == NULL){
        return NULL;
    }
    return leftside+rightside+root->nodeV;
}
