...
int smallestValue(BTNode *node){
    ...
    int leftside = smallestValue(node->left);
    int rightside = smallestValue(node->right);

    highest = leftside > rightside ? rightside:leftside;
    highest = highest > node->item ? node->item:highest;
    return highest;
